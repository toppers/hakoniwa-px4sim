...
さらに，地上に変換された速度  $(u_e, v_e, w_e)$ と，オイラー角の変化率 $(\dot{\phi}, \dot{\theta}, \dot{\psi})$ を時間積分することで，機体の位置 $(x, y, z)$ と姿勢 $(\phi, \theta, \psi)$ が計算できます．
...
地上座標系では並進についてのみ以下のように計算できる（地上座標系での回転はイナーシャの時間変化等のため複雑になるので，等ライブラリでは扱わない）．
