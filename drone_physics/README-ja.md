...
しかし、このライブラリはより汎用的であり、ドローンのシミュレーションプロジェクトで使用できます。
...
|----------|-----------|------|
|`acceleration_in_body_frame` | (1.36),(2.31) | 機体座標系での加速度計算 |
...
1つのローターの推力 $T$ は、ローターの角速度 $\Omega$ の2乗に比例します eq.(2.50)。
$A$ はローターのサイズと空気密度に関連するパラメータです。
$\Omega(t)$ は、デューティー比 $d(t)$ によって制御されます。
書籍の式 (2.48) で記述されている伝達関数 G(s) によって記述され、時間領域の方程式は以下のようになります。
